...
const MAX_SIMULATION_NODES = 200_000;
let _nodes = 0;
...
let _memo: Map<string, BDSMSimu> = new Map();
...
export function calculateBattleProbabilities(player: BDSMPlayer, opponent: BDSMPlayer, debugEnabled: boolean = false):BDSMSimu {
    ...
    function playerTurn(playerHP: any, opponentHP: any, playerShield: any, opponentShield: any, playerStunned: any, opponentStunned: any, playerReflect: any, opponentReflect: any, turns: any): BDSMSimu {
        ...
        if (turns > maxAllowedTurns || ++_nodes > MAX_SIMULATION_NODES) {
            ...
            const points: { [point: number]: number } = { [winPoint]: 0.5 };
            points[lossPoint] = (points[lossPoint] || 0) + 0.5;
            _runs += 1;
            return { points, win: 0.5, loss: 0.5 } as BDSMSimu;
        }
        ...
        if (_memoEnabled) _memo.set(memoKey, mergedResult);
